...
Keep capability framing, durable decision, slice selection, plan readiness, implementation activation, and completion acceptance as separate human approval stages. An agent may record an explicit authorized decision but may not self-approve. One human response may cover an explicitly enumerated homogeneous set of related `DEC-*` requests, one slice's closed packet set at planning, or that same frozen packet set at implementation; record one local entry per artifact, and never extend coverage to new or materially revised members. Implementation requires approved slice selection, approved packet planning, separate implementation approval with exact scoped authority, and a current explicit user instruction to implement the named slice or work.

**Store every approval and reviewer record only in the ignored `.local-codex/approvals/ledger.json`.** Tracked artifacts, registers, histories, summaries, tests, fixtures, and examples must not contain an approver/reviewer identity or label, decision, date, authority reference, reviewed scope, approval metadata, or approval history. Never copy a person's name, account name, display name, email address, or other personal identifier from task context into tracked files.

Reserve planning IDs with `dev-tools/planning/reserve_id.py`. Before implementation, declare bounded `write_scope` and `generated_artifacts`, follow `docs/planning/concurrent-work.md`, and claim the approved packet with `dev-tools/planning/claim_packet.py`. Execute a preauthorized slice packet bundle one packet at a time in dependency order unless separately authorized otherwise; complete the current packet before claiming its dependent successor. An active packet must have one owner, a base revision, durable claim evidence, and no uncoordinated scope overlap. Update its artifact and `docs/planning/register.md` together.
